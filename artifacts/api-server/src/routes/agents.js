...
router.post("/agents", async (req, res) => {
  try {
    ...
    const [agent] = await db.insert(agentsTable).values({
      ...
      isPrivate: isPrivate !== void 0 ? isPrivate : true,
      teeVerified: true
    }).returning();
...
router.post("/agents/:agentId/chat", async (req, res) => {
  try {
    ...
    res.json({
      reply: chosen.reply,
      actionSuggested: chosen.action || void 0,
      confidence,
...
router.get("/agents/:agentId/actions", async (req, res) => {
  try {
    const agentId = parseInt(req.params.agentId);
    const actions = await db.select().from(agentActionsTable).where(eq(agentActionsTable.agentId, agentId)).orderBy(desc(agentActionsTable.createdAt));
    res.json(actions);
  } catch (err) {
...
router.post("/agents/:agentId/actions", async (req, res) => {
  try {
    ...
    await db.update(agentsTable).set({ status: "executing", updatedAt: /* @__PURE__ */ new Date() }).where(eq(agentsTable.id, agentId));
    ...
    setTimeout(async () => {
      ...
      await db.update(agentActionsTable).set({
        ...
        gasUsed: (Math.random() * 1e-3 + 1e-4).toFixed(6) + " ETH",
...
router.get("/agents/:agentId/memory", async (req, res) => {
  try {
    const agentId = parseInt(req.params.agentId);
    const entries = await db.select().from(memoryEntriesTable).where(eq(memoryEntriesTable.agentId, agentId)).orderBy(desc(memoryEntriesTable.createdAt));
    res.json(entries);
  } catch (err) {
