  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-4xl mx-auto">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 bg-card/50 p-6 border border-border/50 backdrop-blur relative overflow-hidden">
        <div className="absolute top-0 right-0 p-32 bg-primary/5 blur-[100px] pointer-events-none"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              return (
                <div 
                  key={p.id}
                  onClick={() => form.setValue("personality", p.id as any)}
        ...
        <div className="space-y-4">
            ...
            {["trading", "social", "payments", "negotiations", "analysis"].map((cap) => {
              ...
              return (
                <div 
                  ...
                  className={`px-4 py-2 border cursor-pointer transition-all text-xs uppercase tracking-widest flex items-center gap-2 ${
                    isSelected 
          ...
          {form.formState.errors.capabilities && (
            <p className="text-xs text-destructive">{form.formState.errors.capabilities.message}</p>
          )}
        </div>
        ...
        <div className="p-4 border border-border/50 bg-background/50 flex items-center justify-between">
          ...
          >
            <div className={`absolute top-0.5 bottom-0.5 w-4 bg-foreground transition-all ${form.watch("isPrivate") ? 'left-[calc(100%-1.125rem)] bg-green-400' : 'left-0.5'}`}></div>
          </div>
        </div>
