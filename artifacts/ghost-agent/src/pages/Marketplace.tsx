    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1,2,3,4,5,6].map(i => (
            <Card key={i} className="border-border/50 bg-card/50 h-56 animate-pulse"></Card>
          ))}
        </div>
      ) : listings?.length === 0 ? (
        <div className="p-16 text-center border border-dashed border-border/50 bg-background/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {listings?.map((listing) => (
            <div key={listing.id} className="group border border-border/50 bg-card/50 backdrop-blur hover:border-secondary/50 transition-all flex flex-col relative overflow-hidden">
              {listing.isVerified && (
                <div className="absolute top-0 right-0 bg-green-400/20 text-green-400 text-[10px] uppercase tracking-widest px-3 py-1 border-b border-l border-green-400/30 flex items-center gap-1 z-10">
