...
function AgentsList() {
  ...
  return <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => <Card key={i} className="border-border/50 bg-card/50 h-48 animate-pulse" />)}
        </div> : agents?.length === 0 ? <div className="flex flex-col items-center justify-center p-16 border border-dashed border-border/50 bg-background/30 text-center">
...
function AgentCard({ agent, onDelete }) {
  return <div className="group relative border border-border/50 bg-card/50 backdrop-blur flex flex-col h-full overflow-hidden hover:border-primary/50 transition-colors">
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-primary/50 to-transparent transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />
      ...
      <div className="p-5 flex-1 flex flex-col gap-4">
        <div className="flex justify-between items-start">
          ...
          <div className={`text-[10px] uppercase tracking-widest px-2 py-1 border ${agent.status === "active" ? "text-green-400 border-green-400/50 bg-green-400/10" : agent.status === "executing" ? "text-primary border-primary/50 bg-primary/10 animate-pulse" : agent.status === "paused" ? "text-yellow-400 border-yellow-400/50 bg-yellow-400/10" : "text-muted-foreground border-border bg-muted/20"}`}>
            {agent.status}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-border/30">
          ...
          {agent.teeVerified && <div className="text-xs px-2 py-1 bg-green-400/10 border border-green-400/30 text-green-400 flex items-center gap-1 ml-auto">
              <Shield size={12} /> TEE
            </div>}
        </div>
      </div>
      ...
      <div className="border-t border-border/50 bg-background/50 p-2 flex justify-between items-center px-4">
        <div className="flex gap-2">
          {agent.capabilities.slice(0, 3).map((cap) => <span key={cap} className="text-[9px] uppercase tracking-wider text-primary bg-primary/10 px-1 py-0.5">
              {cap}
            </span>)}
          {agent.capabilities.length > 3 && <span className="text-[9px] uppercase tracking-wider text-muted-foreground px-1 py-0.5">
